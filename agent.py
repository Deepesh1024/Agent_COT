...
from enhancer import reframe_question
import logging
import os
...
def conversation_agent1():
    i = 0
    presp = []
    print("Initiate the conversation")
    resp = process_audio_transcription()
    while True: 
        if i == 0: 
            print("Which role are you applying for :")
            role = process_audio_transcription()
        ...
        agent3_response = reframe_question(agent1_response.content)
        print(agent3_response)
        presp.append(resp)
        time.sleep(2.5)
        conversation_agent2(role,agent3_response,resp,agenda[i])
        time.sleep(4)
        i += 1
